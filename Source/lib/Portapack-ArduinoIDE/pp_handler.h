 ...
 * https://github.com/htotoo/ESP32-Portapack/blob/main/Source/main/ppi2c/pp_handler.h
 *
...
#ifndef PP_HANDLER_HPP
#define PP_HANDLER_HPP
...
public:
    ...
    static void handleI2CReceive(int numBytes);

    static void init(int scl, int sda, uint8_t addr_, uint32_t freq);
    ...
    static void set_get_shell_data_size_CB(get_shell_data_size_CB cb);    // This will be called when the module asked for shell tx data size
    static void set_got_shell_data_CB(got_shell_data_CB cb);              // This will be called when the PP sent data to the shell
    static void set_send_shell_data_CB(send_shell_data_CB cb);            // This will be called when the module needs to send data to the shell (when prev get_shell_data_size_CB give >0 value)
